...
export const removerPonto = ({ dispatch, commit }, pontoId) => {
  axios
    .delete(`http://localhost/v1/ponto/${pontoId}`)
    .then(() => {
    ...
    .catch(() => {
      dispatch('alert/error', 'Desculpe, Não encontramos este ponto', {
        root: true
      });
    });
};
...
export const atualizarPonto = ({ dispatch, commit }, ponto) =>
  requisicaoAutorizada
    .patch(`http://localhost/v1/ponto/${ponto.id}`, ponto)
    .then(() => {
